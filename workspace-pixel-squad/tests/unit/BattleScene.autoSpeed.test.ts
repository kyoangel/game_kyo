...
// Feature: auto-battle (自動攻擊) runs at 2x speed. Every action's pacing is
...
//                                  which would otherwise leak 2x speed into
...
describe('BattleScene — auto-battle 2x speed', () => {
  let source: string;
  ...
  it('enterAutoMode() speeds the scene up to 2x', () => {
    const body = extractMethod(source, 'enterAutoMode');
    expect(body).toMatch(/this\.setBattleSpeed\(2\)/);
  });
