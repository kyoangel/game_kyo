...
// TitleScene extends Phaser.Scene and cannot be instantiated in the Node
...
const TITLE_SCENE_PATH = resolve(__dirname, '../../../src/scenes/TitleScene.ts');

export function readTitleSceneSource(): string {
  return readFileSync(TITLE_SCENE_PATH, 'utf-8');
}

/**
 * Extracts the full text (signature + body) of a `private` class method
 * from TitleScene.ts source, by locating its declaration and brace-matching
