...
// ResultScene extends Phaser.Scene and cannot be instantiated in the Node
...
export function readResultSceneSource(): string {
  return readFileSync(RESULT_SCENE_PATH, 'utf-8');
}

/**
 * Extracts the full text (signature + body) of a `private` class method
 * from ResultScene.ts source, by locating its declaration and brace-matching
