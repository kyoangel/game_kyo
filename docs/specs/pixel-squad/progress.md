...
- [x] 主角 pilot（2026-07-10 完成，2026-07-10 晚間修正主角外觀）：改用 LPC「ZIP: Split by animation」匯出的乾淨單動畫圖（`public/sprites/party-lpc/protagonist/{walk,slash,hurt,idle}.png`，各 64×64、13欄×4方向列，hurt 無方向、單列6幀）取代舊的 320×320 攤平10×10格 `character_rogue.png`。改了 `src/data/sprites.ts`（新增 `SPRITE_KEYS`/`SPRITE_SHEET_ASSETS`、`LPC_DIRECTION_ROW`、`lpcRowFrameRange()`）和 `BattleScene.ts`（`preload()`/`create()` 的 `animDefs`/`renderParty()`），`SpriteSelection.ts`/`CharacterAnimator.ts` 完全沒改。走完 TDD（先寫失敗測試）+ 152 test files/1400 tests 全綠 + `npm run build` 過 + 實機視覺確認（walk/attack 動畫都正常播放，idle 從單幀升級成真的2幀呼吸循環）+ 獨立 code-reviewer 過。
  - **修正記錄**：第一版誤把 `party-lpc-raw/protagonist-credits.txt`（記載 Male+Kettle helm+Plate armour，跟 Dex 同款）當成主角實際外觀依據，但 `protagonist.png` 本體其實是使用者原本生成的女性劍士（長黑髮、深藍上衣、酒紅褲、長劍）——credits.txt 跟 png 對不上，是先前 session 的 mv 疏失。使用者發現畫面跟原始截圖不符後回報，已重新用正確的 LPC build（`sex=female&hair=Long_straight_black&weapon=Longsword_longsword`...）重新匯出 walk/slash/hurt/idle 四張圖並覆蓋，同時修正 `party-lpc-raw/protagonist-credits.txt` 讓文字跟圖對上。幀版面（13欄×4列）沒變，所以純換圖，程式碼完全沒改，只有測試移除了對舊 `character_rogue.png` 相關 dead code（`SPRITE_KEYS.protagonistSheet`/`protagonistIdle`、`SPRITE_ASSETS`）的 pin 測試。
  - `public/sprites/character_rogue.png` 已確認完全沒被載入（dead asset），但使用者只是「詢問」是否可移除、尚未明確下指令，所以檔案本身還留著，只有程式碼引用清乾淨了——之後使用者明確同意的話再刪。
