...
## 涵蓋範圍與邊界情況
- 我方與敵方隊伍容量一致，皆為 1–5 人（雖然目前 `stages.ts` 部分關卡敵方只配置到 4 人，但版面設計以雙側對稱的 1–5 人為準，垂直排列邏輯（`renderParty` 內的 `topY`/`bottomY`/`n` 計算）維持不變，只改每一列的橫向畫法）。
- 血條顏色改為純陣營色（友軍橘／敵方紫），不再依血量百分比變色；血量資訊改由數字＋填色比例傳達。血量數字最高視為 4 位數（9999），`labelX` 區塊的文字寬度需以此為準抓版面，不需要為更大的數字預留空間。
