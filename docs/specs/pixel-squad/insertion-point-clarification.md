...
## Rules
- The insertion point is fixed at `scenes/BattleScene.ts:117`.
...
## Acceptance Criteria
- Given a new feature requires initialization of `this.gameState` data structures, When the implementation spec is written, Then it must explicitly state: "Insert new logic after `this.gameState = data.gameState` at `scenes/BattleScene.ts:117`."